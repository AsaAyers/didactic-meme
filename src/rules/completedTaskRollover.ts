 ...
 *
 * Finds every checked task that was completed **today** (i.e. its `done:`
 * field equals today's date, as stamped by `stampDone`) and that has not yet
 * been processed (no `copied:1` marker).  For each such task the rule:
 *
 *   1. Appends `copied:1` to the completed task so it is not re-processed on
 *      subsequent pipeline runs (idempotency marker).
 *   2. Inserts a fresh incomplete copy of the task directly after it.  If the
 *      task carries a `repeat:` schedule the clone's date fields (due, start,
 *      snooze) are advanced according to that schedule; otherwise the clone
 *      inherits the original dates unchanged.  The `done:` field is not
