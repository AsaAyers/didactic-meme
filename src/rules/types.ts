...
export type RuleContext = {
  ...
  /**
   * A glob pattern or array of glob patterns / relative file paths (relative to
   * vaultPath) that restricts which files each rule processes. When provided,
   * every rule's resolved source list is filtered to contain only files that
   * match at least one of the patterns. All rules (including transitive
   * dependencies) are still executed; only the set of files they operate on is
   * narrowed.
   */
