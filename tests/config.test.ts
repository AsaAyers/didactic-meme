...
import { loadConfig, getDefaultConfig, CONFIG_FILENAME } from '../src/config.js';
...
const SPEC_B: RuleSpec = {
  name: 'specB',
  sources: [{ type: 'glob', pattern: 'notes/**/*.md', exclude: ['archive/**'] }],
  query: { type: 'tasks' },
  actions: [],
};
...
describe('loadConfig', () => {
  ...
  it('returns the stored config when it already contains all known rules', async () => {
    const customSources = [{ type: 'glob' as const, pattern: 'custom/**/*.md' }];
    const initial = {
  ...
  it('merges default sources for rules missing from an outdated config', async () => {
    ...
    const initial = { specA: { sources: [{ type: 'glob', pattern: '**/*.md' }] } };
    await fs.writeFile(configPath(), JSON.stringify(initial), 'utf-8');
  ...
  it('throws a descriptive error when the config contains invalid JSON', async () => {
    ...
    await expect(loadConfig(tempVault, [SPEC_A])).rejects.toThrow(CONFIG_FILENAME);
  });
